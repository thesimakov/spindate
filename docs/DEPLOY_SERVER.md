...
### Если `git pull` ругается на `next-env.d.ts`

Файл **`next-env.d.ts`** Next.js пересобирает при `npm run build`, из‑за этого на сервере появляются **локальные отличия** от Git, и merge блокируется.

Перед `git pull` сбрось только этот файл к версии из репозитория (или к последнему коммиту), затем тяни код:
...
После успешного pull снова собери проект (см. раздел 2 ниже).
